...
class Scraper:
    ...
    def getProductLinks(self, productSelector, pages=0, productCount=250, paginated=False, buttonSelector=''):
        ...
        if paginated == True:
            for page in range(0,pages):
                ...
                self.browser.find_elements(By.XPATH, buttonSelector).click()     
        else:
            _elements = (self.browser.find_elements(By.XPATH, productSelector))
            for element in _elements:
                    elements.append(element)
        ...
        for we in elements:
            ...
            self.links.append(we.get_property('href'))
  
    def scrape(self, brandName, nameSelector, idSelector, compositionSelector, compositionSplitBy, priceSelector, delay=5):
...
scraper.getProductLinks('//h3[@class="item-heading"]//a[@class="link"]')
scraper.scrape('H&M',
...
scraper.getProductLinks('//a[@class="plp-product-link"]')
